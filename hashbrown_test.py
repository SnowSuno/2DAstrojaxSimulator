...
def catch(va=0, vb=0, vc=0, A0=(0, 0), B0=(300, 0), C0=(150, 180), duration=20, step=0.001):
    ...
    D = rk4(0, duration, step, ((A0[0], A0[1], B0[0], B0[1], C0[0], C0[1]), (Ax, Ay, Bx, By, Cx, Cy)),
            'Catch_Va %d_Vb %d_Vc %d' % (va, vb, vc))
...
def doublePendulum(m1=1, m2=1, l1=1, l2=1, a0=0, b0=0, wa0=0, wb0=10, g=9.8, time=15, step=0.0013):
    ...
    def f3_wa(t, a, b, wa, wb):
        return -1 * (m1 * g * sin(a) + m2 * sin(a - b) * (
                    g * cos(b) + l1 * cos(a - b) * (pow(wa, 2)) + l2 * (pow(wb, 2)))) / (
                           l1 * (m1 + m2 * (pow((sin(a - b)), 2))))

    def f4_wb(t, a, b, wa, wb):
        return (sin(a - b) * (
                    (m1 + m2) * g * cos(a) + (m1 + m2) * l1 * (pow(wa, 2)) + m2 * l2 * cos(a - b) * (pow(wb, 2)))) / (
                           l1 * (m1 + m2 * (pow((sin(a - b)), 2))))
    ...
    D = rk4(0, time, step, ((a0, b0, wa0, wb0), (F1, F2, F3, F4)),
            'Double Pendulum wa0_%d wb0_%d timestep_%fs' % (wa0, wb0, step))
    D1 = D.values
    D2 = []
